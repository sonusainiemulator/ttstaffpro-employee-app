...
class _AppealsListScreenState extends State<AppealsListScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final result = await _repository.getAppeals(
        skip: 0,
        take: _pageSize,
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
        ...
        !_isLoadingMore &&
        !_isLoading) {
      if (_appeals.length < _totalCount) {
        _loadMore();
      }
    }
  }
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _buildFilterChip(language.lblPending, tempStatus == 'pending', () {
                      setModalState(() => tempStatus = tempStatus == 'pending' ? null : 'pending');
                    }),
                    _buildFilterChip(language.lblUnderReview, tempStatus == 'under_review', () {
                      setModalState(() => tempStatus = tempStatus == 'under_review' ? null : 'under_review');
                    }),
                    _buildFilterChip(language.lblApproved, tempStatus == 'approved', () {
                      setModalState(() => tempStatus = tempStatus == 'approved' ? null : 'approved');
                    }),
                    _buildFilterChip(language.lblRejected, tempStatus == 'rejected', () {
                      setModalState(() => tempStatus = tempStatus == 'rejected' ? null : 'rejected');
                    }),
                    _buildFilterChip(language.lblWithdrawn, tempStatus == 'withdrawn', () {
                      setModalState(() => tempStatus = tempStatus == 'withdrawn' ? null : 'withdrawn');
                    }),
                  ],
                ),
                const SizedBox(height: 20),
  ...
  Widget _buildAppealCard(WarningAppeal appeal) {
    return Container(
            ...
            children: [
              Row(
                children: [
                  ...
                  _buildStatusBadge(appeal.status, appeal.statusLabel),
                ],
              ),
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      'Filters: ${_selectedStatus?.replaceAll('_', ' ').capitalize ?? 'All'}',
                                      style: TextStyle(
                                        fontSize: 13,
                                        fontWeight: FontWeight.w500,
                                        color: appStore.isDarkModeOn
  ...
  Widget _buildContent() {
    if (_isLoading && _appeals.isEmpty) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(
                language.lblErrorLoadingAppeals,
                style: TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
                  color: appStore.isDarkModeOn
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                language.lblNoAppealsDescription,
                style: TextStyle(
                  fontSize: 14,
                  color: appStore.isDarkModeOn
                      ? Colors.grey[400]
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        padding: const EdgeInsets.all(16),
        itemCount: _appeals.length + (_isLoadingMore ? 1 : 0),
        itemBuilder: (context, index) {
